...
- Power-ups con `kind`: `'boost'` (velocidad x2, 2 chevrones naranja), `'triple'` (disparo 3 balas, 3 chevrones cian) y `'shield'` (glifo azul, escudo). Aparecen al destruir asteroides (12% por asteroide, tipo al azar entre los 3, máx. 1 de cada `kind` en pantalla, `POWERUP_TTL = 10`). Deben tener `radius` asignado (`POWERUP_RADIUS`); sin eso la recogida evalúa `NaN` y nunca se activa.
...
- El triple disparo vive en `ship.tripleTimer` (segundos, `TRIPLE_DURATION = 5`); activo, `Ship.tryShoot()` devuelve 3 balas en abanico (`TRIPLE_SPREAD = 0.17` rad). Se limpia en `reset()` igual que el boost. El HUD lo muestra como `TRIPLE SHOT (s)`.
